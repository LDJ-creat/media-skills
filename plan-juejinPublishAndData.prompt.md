...
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\SKILL.md — 复用 storageState 优先、output/normalized 说明、check-login/fetch 工作流写法
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\scripts\common.ts — 复用 CLI 参数、认证文件发现、输出目录解析模式
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\scripts\normalize.ts — 复用 raw 去重与 normalized 产物分层思路
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\references\output-format.md — 参考如何把 records、normalized、metrics 的边界写清楚
- c:\Users\FLDJ\Desktop\skills\baoyu-post-to-wechat\SKILL.md — 参考发布类 skill 的触发描述、前置检查和文章处理 workflow
