...
function resolveLocalFilePath(inputPath: string, baseDir?: string): string {
  if (!inputPath) return inputPath;
  if (inputPath.startsWith("http://") || inputPath.startsWith("https://")) return inputPath;
  if (path.isAbsolute(inputPath)) return inputPath;

  const baseCandidate = baseDir ? path.resolve(baseDir, inputPath) : "";
  if (baseCandidate && fs.existsSync(baseCandidate)) return baseCandidate;

  const cwdCandidate = path.resolve(process.cwd(), inputPath);
  if (fs.existsSync(cwdCandidate)) return cwdCandidate;

  const parentCandidate = findFileInParents(process.cwd(), inputPath);
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(filePath) && !path.isAbsolute(args.filePath)) {
    const found = findFileInParents(process.cwd(), args.filePath);
  ...
  const rawCoverPath = args.cover ||
    ...
    frontmatter.cover ||
    frontmatter.image;
  const coverPath = rawCoverPath ? resolveLocalFilePath(rawCoverPath, baseDir) : rawCoverPath;
