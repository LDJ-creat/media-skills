...
export function loadExtendConfig(): Partial<ExtendConfig> {
  const paths = [
    path.join(process.cwd(), ".config", "baoyu-markdown-to-html", "EXTEND.md"),
    path.join(process.env.XDG_CONFIG_HOME || path.join(homedir(), ".config"), "baoyu-markdown-to-html", "EXTEND.md"),
  ];
