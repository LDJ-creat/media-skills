...
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\SKILL.md — 复用 storageState 优先、手动执行流程、输出目录和 normalized 说明
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\scripts\common.ts — 复用认证发现、路径解析、CLI 参数模式
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\scripts\types.ts — 参考如何定义 raw capture、normalized summary、daily rows 等类型层次
- c:\Users\FLDJ\Desktop\skills\get-wechat-data\references\config\first-time-setup.md — 复用跨平台与 headless 前置说明
- c:\Users\FLDJ\Desktop\skills\baoyu-post-to-wechat\SKILL.md — 参考发布类 skill 的工作流组织和文章输入处理逻辑
